...
export class AchievementsComponent {
  ...
  // FIX: Was plain property — OnPush won't detect changes. Now a signal.
  ...
  nextCert(event?: Event) {
    ...
    const current = this.selectedCertIndex();
    if (current !== null) {
      this.selectedCertIndex.set((current + 1) % this.certificates.length);
    }
  }
  ...
  prevCert(event?: Event) {
    ...
    const current = this.selectedCertIndex();
    if (current !== null) {
      this.selectedCertIndex.set(
        (current - 1 + this.certificates.length) % this.certificates.length
      );
    }
  }
  ...
  handleKeyboardEvent(event: KeyboardEvent) {
    ...
    if (event.key === 'Escape') this.closeCert();
    else if (event.key === 'ArrowRight') this.nextCert();
    else if (event.key === 'ArrowLeft') this.prevCert();
  }
  ...
  private handleSwipe() {
    if (this.selectedCertIndex() === null) return;
    const threshold = 50;
    if (this.touchEndX < this.touchStartX - threshold) this.nextCert();
    if (this.touchEndX > this.touchStartX + threshold) this.prevCert();
  }
}
